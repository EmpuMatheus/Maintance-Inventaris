...
interface ReportFiltersProps {
  ...
  onChange: (patch: Partial<InventoryReportFilters>) => void;
  onReset: () => void;
}
...
export default function ReportFilters({ value, onChange, onReset }: ReportFiltersProps) {
  ...
  const { data: subcategories } = useQuery({
    queryKey: ['master', 'subcategories', value.categoryId],
    queryFn: () => listMaster('subcategories', { categoryId: value.categoryId }),
  ...
  const set = (patch: Partial<InventoryReportFilters>) => {
    if ('categoryId' in patch && patch.categoryId !== value.categoryId) patch.subcategoryId = undefined;
  ...
  return (
    <div className="mb-4 space-y-3 rounded-lg border border-slate-200 bg-white p-3">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <select value={value.subcategoryId ?? ''} onChange={(e) => set({ subcategoryId: e.target.value || undefined })} disabled={!value.categoryId} className={`${selectClass()} disabled:bg-slate-50`}>
          <option value="">All Subcategories</option>
          {rows(subcategories).map((r) => <option key={r.id} value={r.id}>{r.name}</option>)}
        </select>
      </div>

      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <select value={value.brandId ?? ''} onChange={(e) => set({ brandId: e.target.value || undefined })} className={selectClass()}>
          <option value="">All Brands</option>
          {rows(brands).map((r) => <option key={r.id} value={r.id}>{r.name}</option>)}
        </select>
      ...
      <div className="grid grid-cols-1 items-end gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="flex items-center gap-2">
          <input
            type="date"
            value={value.purchaseDateFrom ?? ''}
            onChange={(e) => set({ purchaseDateFrom: e.target.value || undefined })}
          ...
          <input
            type="date"
            value={value.purchaseDateTo ?? ''}
            onChange={(e) => set({ purchaseDateTo: e.target.value || undefined })}
            className={`${selectClass()} w-full`}
