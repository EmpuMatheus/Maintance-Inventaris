...
import type { MaintenanceReportFilters } from '../types';
...
interface MaintenanceReportFiltersProps {
  value: MaintenanceReportFilters;
  onChange: (patch: Partial<MaintenanceReportFilters>) => void;
  onReset: () => void;
}

export default function MaintenanceReportFilters({ value, onChange, onReset }: MaintenanceReportFiltersProps) {
  ...
  const set = (patch: Partial<MaintenanceReportFilters>) => {
    ...
    <div className="mb-4 space-y-3 rounded-lg border border-slate-200 bg-white p-3">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="relative lg:col-span-2">
          ...
          <input
            type="text"
            placeholder="Search code, asset, problem..."
            value={value.keyword ?? ''}
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <select value={value.technicianId ?? ''} onChange={(e) => set({ technicianId: e.target.value || undefined })} className={selectClass()}>
