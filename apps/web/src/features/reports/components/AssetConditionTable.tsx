...
import type { AssetConditionItem, PaginationMeta } from '../types';
...
interface AssetConditionTableProps {
...
export default function AssetConditionTable({
  items,
  isLoading,
  isError,
  errorMessage,
        ...
        <table className="w-full text-left text-sm">
          <thead className="border-b border-slate-200 bg-slate-50">
            <tr>
              ...
              <th className="px-4 py-3 font-medium text-slate-600">Last Change</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {renderState(SORTABLE.length + 5)}
            {!isLoading && !isError && items.map((item) => (
              <tr key={item.id} className="cursor-pointer hover:bg-slate-50" onClick={() => navigate(`/assets/${item.id}`)}>
                <td className="px-4 py-3 font-mono text-xs text-indigo-600">{item.assetCode}</td>
                ...
                <td className="max-w-48 px-4 py-3 text-slate-600">{item.location || '-'}</td>
                <td className="px-4 py-3 text-slate-600">{item.pic || '-'}</td>
                <td className="px-4 py-3 text-xs text-slate-500">
                  {item.lastConditionChange ? formatDate(item.lastConditionChange) : '-'}
                  {item.previousCondition && item.previousCondition !== item.condition && ` · ${item.previousCondition.replace(/_/g, ' ')} →`}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="space-y-3 md:hidden">
        ...
        {!isLoading && !isError && items.map((item) => (
          <button key={item.id} onClick={() => navigate(`/assets/${item.id}`)} className="block w-full rounded-lg border border-slate-200 bg-white p-4 text-left">
            ...
            <p className="mt-1 text-xs text-slate-500">{item.category || '-'} · {item.department || '-'} · {item.pic || '-'}</p>
            <p className="text-xs text-slate-400">
              📍 {item.location || '-'} · Last change {item.lastConditionChange ? formatDate(item.lastConditionChange) : '-'}
              {item.previousCondition && item.previousCondition !== item.condition ? ` (${item.previousCondition.replace(/_/g, ' ')} →)` : ''}
            </p>
          </button>
        ))}
